...
vgm_writer::vgm_writer(running_machine &machine) :
	m_chip_id(0)
{
	// start the timer
	...
	hMachine = &machine;
	m_timer = machine.scheduler().timer_alloc(timer_expired_delegate(FUNC(vgm_writer::vgmfile_callback), this));
	m_timer->adjust(attotime::from_hz(0), 0, attotime::from_hz(0));
}

void vgm_writer::vgm_start(char* vgmname)
{
	...
	*/
	...
	gamedrv = &hMachine->system();
	devimg = image_interface_iterator(hMachine->root_device()).first();
	/*
	if (gamedrv)
	...
	*/
	...
	wcs2utf16(VgmTag.strAuthorNameE, L"MAmidiMEmo", 0x30);
	wcs2utf16(VgmTag.strAuthorNameJ, L"MAmidiMEmo", 0x10);
...
TIMER_CALLBACK_MEMBER(vgm_writer::vgmfile_callback)
{
	uint16_t curvgm;
 ...
 void vgm_writer::vgm_header_postwrite(uint16_t vgm_id)
{
	VGM_INF* VI;
	VGM_HEADER* Header;
	VGM_ROM_DATA* VR;
...
void vgm_writer::vgm_header_sizecheck(uint16_t vgm_id, uint32_t MinVer, uint32_t MinSize)
{
	VGM_INF* VI;
	VGM_HEADER* Header;
...
void vgm_writer::vgm_header_clear(uint16_t vgm_id)
{
	VGM_INF* VI;
	VGM_HEADER* Header;
...
void vgm_writer::vgm_open(uint8_t chip_type, int clock)
{
	uint16_t chip_id;
	uint16_t chip_file;
	uint16_t curvgm;
	uint32_t chip_val;
	char vgm_name[_MAX_PATH];
	uint8_t use_two;
	...
	if (LOG_VGM_FILE != 0xDD)
	{
		...
		if (chip_type == VGMC_ES5506)
			return;	// logging needs to be reworked for this one
		if (chip_type == VGMC_SCSP)
			return;	// timing is horribly off (see Fighting Vipers)
		else if (chip_type == VGMC_WSWAN)
			return;	// WonderSwan emulation is horrible in general - even the logs are screwed
	}
...
void vgm_writer::vgm_setup_pcmcache(VGM_PCMCACHE* TempPC, uint32_t Size)
{
...
void vgm_writer::vgm_header_set(uint8_t attr, uint32_t data)
{
	VGM_HEADER* VH;
	uint8_t bitcnt;
	
	if (! LOG_VGM_FILE || m_chip_id == 0xFFFF)
		return;
	if (VgmChip[m_chip_id].ChipType == 0xFF)
		return;
	
	VH = &VgmFile[VgmChip[m_chip_id].VgmID].Header;
	switch(VgmChip[m_chip_id].ChipType & 0x7F)	// Write the Header data
	{
	case VGMC_SN76496:
	case VGMC_T6W28:
		switch(attr)
	...
	case VGMC_YMF262:
		...
		case 0x00:	// is Part of OPL4
			if (data)
			{
				VgmChip[m_chip_id].ChipType = VGMC_YMF278B;
				VH->lngHz262 = 0x00;
			}
			break;
		}
...
void vgm_writer::vgm_close(uint16_t vgm_id)
{
	VGM_INF* VI;
	VGM_HEADER* Header;
...
void vgm_writer::vgm_write_delay(uint16_t vgm_id)
{
	VGM_INF* VI;
	uint16_t delaywrite;
...
void vgm_writer::vgm_write(uint8_t port, uint16_t r, uint8_t v)
{
	VGM_CHIP* VC;
	VGM_INF* VI;
	VGM_PCMCACHE* VPC;
	...
	if (! VC->HadWrite)
	{
		...
		if (VC->ChipType & 0x80)
		{
			for (curchip = 0x00; curchip < m_chip_id; curchip ++)
			{
...
void vgm_writer::vgm_write_large_data(uint16_t chip_id, uint8_t type, uint32_t datasize, uint32_t value1, uint32_t value2, const void* data)
{
...
uint8_t vgm_writer::vgm_nes_ram_check(VGM_INF* VI, uint32_t datasize, uint32_t* value1, uint32_t* value2, const uint8_t* data)
{
	uint16_t CurPos;
	uint16_t DataStart;
	uint16_t DataEnd;
...
uint16_t vgm_writer::vgm_get_chip_idx(uint8_t chip_type, uint8_t Num)
{
...
void vgm_writer::vgm_flush_pcm(VGM_CHIP* VC)
{
	VGM_INF* VI;
	VGM_PCMCACHE* VPC;
	uint8_t SingleWrt;
...
void vgm_writer::vgm_change_rom_data(uint32_t oldsize, const void* olddata, uint32_t newsize, const void* newdata)
{
	VGM_INF* VI;
	VGM_ROM_DATA* VR;
	uint16_t curvgm;
...
void vgm_writer::vgm_dump_sample_rom(uint16_t chip_id, uint8_t type, memory_region* region)
{
	if (region == nullptr)
		return;
...
void vgm_writer::vgm_dump_sample_rom(uint16_t chip_id, uint8_t type, address_space& space)
{
...
#pragma warning(disable: 4505)
 ...
 size_t vgm_writer::str2utf16(gd3char_t* dststr, const char* srcstr, size_t max)
{
	size_t srclen;
	size_t wlen;
	wchar_t* wstr;
...
#pragma warning(pop)
 ...
 size_t vgm_writer::wcs2utf16(gd3char_t* dststr, const wchar_t* srcstr, size_t max)
{
	size_t wlen;
...
#define LO_SURROGATE_START	0xDC00
 ...
 size_t vgm_writer::utf16len(const gd3char_t* str)
{
