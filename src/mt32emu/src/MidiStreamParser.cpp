...
Bit32u MidiStreamParserImpl::parseShortMessageDataBytes(const Bit8u stream[], Bit32u length) {
	...
	while ((streamBufferSize < shortMessageLength) && (length-- > 0)) {
		...
		if (dataByte < 0x80) {
			...
			char s[128];
			sprintf_s(s, "parseShortMessageDataBytes: Invalid short message: status %02x, expected length %i, actual %i -> ignored", *streamBuffer, shortMessageLength, streamBufferSize);
